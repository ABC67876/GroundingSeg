...
# load_from = "biomedbert/mask2formerv3/det/best_coco_bbox_mAP_epoch_11.pth"
# load_from = "brainptm/aug.pth"
load_from = "new-brainptm/det/best_coco_bbox_mAP_epoch_7.pth"
...
model = dict(
    ...
    data_preprocessor=dict(
        ...
        mean=[70.49, 70.49, 70.49],
        std=[87.20, 87.20, 87.20],
        bgr_to_rgb=False,
        pad_mask=False,
    ),
    language_model=dict(
...
train_dataloader = dict(
    ...
    dataset=dict(
        type="ConcatDataset",
        datasets=[
            # _base_.oasis4_train_dataset,
            # _base_.oasis35_train_dataset,
            # _base_.adni35_train_dataset,
            _base_.brainptm_train_dataset,
        ],
    ),
)
...
test_dataloader = dict(
    ...
    dataset=dict(
        type="ConcatDataset",
        datasets=[
            # _base_.adni35_test_dataset,
            _base_.brainptm_test_dataset,
        ],
    ),
)
...
val_dataloader = dict(
    ...
    dataset=dict(
        type="ConcatDataset",
        datasets=[
            # _base_.adni35_valid_dataset,
            _base_.brainptm_valid_dataset,
        ],
    ),
)
...
val_evaluator = dict(
    ...
    ann_file=_base_.brainptm_data_root + "valid/annotations_without_background.json",
    seg_prefix=_base_.brainptm_data_root+ "valid/annotations_without_background/",
)
test_evaluator = dict(
    type="CocoPanopticMetric",
    ann_file=_base_.brainptm_data_root + "test/annotations_without_background.json",
    seg_prefix=_base_.brainptm_data_root + "test/annotations_without_background/",
)

max_epoch = 20

default_hooks = dict(
    checkpoint=dict(interval=1, max_keep_ckpts=10, save_best="auto", rule="greater"),
