...
class BaseDetDataset(BaseDataset):
    ...
    def full_init(self) -> None:
        ...
        save_path = self.ann_file.replace(".json", ".npz")
        if os.path.exists(save_path) and not True:
            if not self.serialize_data:
                raise NotImplementedError(
                    "The serialized data should not be loaded when "
                    "``serialize_data=False``."
                )
            self.data_bytes = np.load(save_path, allow_pickle=True)["data_bytes"]
            self.data_address = np.load(save_path, allow_pickle=True)["data_address"]
        ...
        if not os.path.exists(save_path) and True:
            np.savez(
                save_path, data_bytes=self.data_bytes, data_address=self.data_address
            )
