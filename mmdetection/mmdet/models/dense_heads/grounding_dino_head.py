...
class GroundingDINOHead(DINOHead):
    ...
    def _predict_by_feat_single(
        ...
        results.indices = bbox_index  # FIXME 图省事先改到既有文件里了，需要重构。
        return results
    ...
    def loss(
        ...
        if need_results:
            ...
            losses, predictions = self.loss_by_feat(
                *loss_inputs,
                batch_token_positive_maps=batch_token_positive_maps,
                need_results=True,
            )
    ...
    # 原本loss_by_feature是在DINOHead里的，但是这里需要重写
    def loss_by_feat(
        self,
        all_layers_cls_scores: Tensor,
        all_layers_bbox_preds: Tensor,
        ...
        # following 2 params are added for pred
        batch_token_positive_maps: Optional[List[dict]] = None,
        need_results: bool = False,  # when used in seg
    ) -> Dict[str, Tensor]:
        """Loss function.
        ...
        if need_results:
            for img_id in range(len(batch_img_metas)):
                cls_score = all_layers_matching_cls_scores[-1][img_id]
                bbox_pred = all_layers_matching_bbox_preds[-1][img_id]
                img_meta = batch_img_metas[img_id]
                token_positive_maps = batch_token_positive_maps[img_id]
                results = self._predict_by_feat_single(
                    cls_score, bbox_pred, token_positive_maps, img_meta, rescale=False
                )
                result_list.append(results)
