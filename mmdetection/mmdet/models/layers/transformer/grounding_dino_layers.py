...
class GroundingDinoTransformerDecoderLayer(DeformableDetrTransformerDecoderLayer):
    ...
    def forward(
        ...
        return_position_dict: bool = False,
        **kwargs,
    ) -> Tensor:
        ...
        if return_position_dict:
            query, deformable_positions_dict = self.cross_attn(
                query=query,
                key=key,
                value=value,
                query_pos=query_pos,
        ...
        if return_position_dict:
            return query, deformable_positions_dict
        else:
            return query
...
# the following class is used after pixel encoder and before segment head
# it's designed to merge text into image again
class FuseBeforeSeg(DeformableDetrTransformerEncoder):
    def __init__(
        self, cross_attn_cfg: ConfigType, **kwargs
    ) -> None:
    ...
    def forward(
        ...
        Args:
            ...
            position_ids (Tensor, optional): Text position ids.
                Defaults to None.
        """
        assert spatial_shapes.shape[0] == 1, "Only support single scale"
        output = query
        reference_points = self.get_encoder_reference_points(
            spatial_shapes, valid_ratios, device=query.device
        )
        ...
        # main process
        for layer_id, layer in enumerate(self.layers):
            output = self.cross_attn_layers[layer_id](
                query=output,
                query_pos=query_pos,
                key=memory_text,
                value=memory_text,
