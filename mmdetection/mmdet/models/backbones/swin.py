...
class SwinTransformer(BaseModule):
    ...
    def __init__(
        ...
        layer = build_norm_layer(norm_cfg, self.num_features[0])[1]
        layer_name = f"norm{0}"
        self.add_module(layer_name, layer)
    ...
    def forward(self, x):
        ...
        src_prj = x.view(-1, *hw_shape, 96).permute(0, 3, 1, 2).contiguous()

        outs = []
        for i, stage in enumerate(self.stages):
            x, hw_shape, out, out_hw_shape = stage(x, hw_shape)

            if i in self.out_indices or i == 0:
