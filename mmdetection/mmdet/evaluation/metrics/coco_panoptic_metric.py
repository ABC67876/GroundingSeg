...
class CocoPanopticMetric(BaseMetric):
    ...
    def __init__(
        ...
        # add for the modified metrics
        self.new_metrics_result = []
    ...
    def _compute_batch_pq_stats(
        self, data_samples: Sequence[dict], is_new_metrics=False
    ):
        ...
        ``self.results``.
        """
        if isinstance(self.dataset_meta, list):
            raise NotImplementedError("concate dataset is not supported, mainly because of different prefx")
            for i in range(1,len(self.dataset_meta)):
                for key in self.dataset_meta[i]:
                    if key!='palette' and self.dataset_meta[i][key]!=self.dataset_meta[0][key]:
                        print(key)
                        print(self.dataset_meta[0][key])
                        print(self.dataset_meta[1][key])
                        raise RuntimeError("concate dataset should have same meta info")
            self.dataset_meta = self.dataset_meta[0]
        ...
        for data_sample in data_samples:
            ...
            if is_new_metrics:
                onehot_pred, onehot_gt = pq_compute_single_core(
                    proc_id=0,
                ...
                dsc = monaimetrics.compute_dice(
                    onehot_pred,
                    onehot_gt,
                    include_background=False,
                )
                nsd = monaimetrics.compute_surface_dice(
                    onehot_pred,
                    onehot_gt,
                    class_thresholds=[1.0]*(onehot_pred.shape[1]-1),
                    include_background=False,
                )
                self.new_metrics_result.append([segm_file, iou, nsd, dsc])
            else:
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        for i in range(1, len(pq_stat.pq_per_cat) + 1):
            print("class " + str(i), end=" ")
            print("num: " + str(pq_stat[i].tp + pq_stat[i].fn), end=" ")
            print(
                "precision: "
                + str(pq_stat[i].tp / (pq_stat[i].tp + pq_stat[i].fp + 1e-5))[:6],
                end=" ",
            )
            print(
                "recall: "
                + str(pq_stat[i].tp / (pq_stat[i].tp + pq_stat[i].fn + 1e-5))[:6],
                end=" ",
            )
            print(
                "IoU(TP): " + str(pq_stat[i].iou / (pq_stat[i].tp + 1e-5))[:6],
                end=" ",
            )
            print(
                "DSC(TP): " + str(pq_stat[i].dsc / (pq_stat[i].tp + 1e-5))[:6],
                end=" ",
            )
            print(
                "IoU: "
                + str(
                    pq_stat[i].iou
                    / (pq_stat[i].tp + pq_stat[i].fn + pq_stat[i].fp + 1e-5)
                )[:6],
                end=" ",
            )
            print(
        ...
        metrics = [("All", None), ("Things", True), ("Stuff", None)]
        pq_results = {}
